...
const App = {
    ...
    init() {
        ...
        if (localStorage.getItem('sdrg-darkmode') === 'true') {
    ...
    loadProgress() {
        try {
            const saved = localStorage.getItem('sdrg-progress');
            if (saved) this.progress = JSON.parse(saved);
            const savedQuiz = localStorage.getItem('sdrg-quiz-results');
    ...
    saveProgress() {
        localStorage.setItem('sdrg-progress', JSON.stringify(this.progress));
        localStorage.setItem('sdrg-quiz-results', JSON.stringify(this.quizResults));
    },
        ...
        let html = `
            <div class="fade-in">
                <div class="dashboard-hero">
                    <h2>SDRG Academy へようこそ</h2>
                    <p>Study Data Reviewer's Guide（SDRG）の作成スキルを基礎から実践まで体系的に学べるプラットフォームです。</p>
                </div>
    ...
    showCompletionMessage() {
        ...
        modal.innerHTML = `
            ...
            <p>
                全モジュールを完了しました！<br>
                SDRGの基礎から実践まで一通り学びました。<br>
                実際のプロジェクトで高品質なSDRGを作成しましょう！
            </p>
    ...
    toggleDarkMode() {
        ...
        if (isDark) {
            ...
            localStorage.setItem('sdrg-darkmode', 'false');
        } else {
            document.documentElement.setAttribute('data-theme', 'dark');
            localStorage.setItem('sdrg-darkmode', 'true');
        }
    },
    ...
    resetProgress() {
        if (confirm('全ての学習進捗をリセットしますか？この操作は元に戻せません。')) {
            this.progress = {};
            this.quizResults = {};
            localStorage.removeItem('sdrg-progress');
            localStorage.removeItem('sdrg-quiz-results');
            this.buildSidebar();
            this.updateGlobalProgress();
            this.showDashboard();
        }
